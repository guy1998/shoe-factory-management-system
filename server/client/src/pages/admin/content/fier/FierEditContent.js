...
import { getFierStatisticById, editFierStatistic } from "./scripts/fier-scripts";
...
function FierEditContent() {
  ...
  useEffect(() => {
    getFierStatisticById(notification, navigate, id).then((data) => {
      if (data) {
        setStat(data);
        setSelectedProducts(data.products.map(product=>{
            return {
                code: product.code,
        ...
        <MDButton
          ...
          onClick={() => {
            if (selectedProducts.length) {
              editFierStatistic(notification, navigate, selectedProducts, id);
            } else {
...
export default FierEditContent;
